...
// Dividend events are shared market data — manual creation/update disabled to prevent
// any user from altering records visible to all users. Events are populated exclusively
// by the sync service (Yahoo/Polygon). Deletion also disabled.
...
// Backward compat: GET / returns events
router.get('/', requireAuth, getEventsHandler);
// POST / (manual event creation) disabled — shared market data, see above
