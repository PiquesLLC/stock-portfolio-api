...
router.post('/link-token', mutationLimiter, requireAuth, requirePlan('pro'), requireMfaAssurance, createLinkTokenHandler);

// POST /plaid/exchange-token - Exchange public token for access token
router.post('/exchange-token', mutationLimiter, requireAuth, requirePlan('pro'), requireMfaAssurance, exchangeTokenHandler);

// GET /plaid/items - List linked accounts (read-only, no MFA step-up needed)
router.get('/items', requireAuth, requirePlan('pro'), getItemsHandler);

// GET /plaid/items/:itemId/holdings - Fetch holdings from linked brokerage
router.get('/items/:itemId/holdings', requireAuth, requirePlan('pro'), getHoldingsHandler);

// POST /plaid/items/:itemId/sync - Re-sync holdings from linked brokerage
router.post('/items/:itemId/sync', mutationLimiter, requireAuth, requirePlan('pro'), syncHoldingsHandler);

// DELETE /plaid/items/:itemId - Disconnect a linked account (sensitive — requires MFA)
router.delete('/items/:itemId', mutationLimiter, requireAuth, requirePlan('pro'), requireMfaAssurance, disconnectItemHandler);
