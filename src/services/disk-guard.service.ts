 ...
 *
 * VACUUM only runs with the same `free >= 1.2x DB` headroom guard the admin
 * cleanup endpoint uses (the Apr-26 incident showed VACUUM at tight disk hangs).
 */
...
// Cooldown so a sustained outage doesn't spam alerts (matches v2 reconcile cron pattern).
let lastAlertAt = 0;
...
export async function runDiskGuard(): Promise<void> {
  ...
  // 2) At CRITICAL: aggressive prune + reclaim (VACUUM, freeing the redundant backup if needed).
  ...
  if (after && after.usedPercent >= config.diskCriticalPct) {
    const now = Date.now();
    if (now - lastAlertAt >= config.diskAlertCooldownMs) {
      lastAlertAt = now;
