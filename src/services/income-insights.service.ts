...
export async function getIncomeInsights(userId: string, window: IncomeWindow = 'today', portfolioId?: string): Promise<IncomeInsightsResponse> {
  const cacheKey = `income-insights:${userId}:${window}${portfolioId ? `:${portfolioId}` : ''}`;
