...
2. Add a guard at every v1 write site in `src/services/creator-billing.service.ts`:
   - Top of `creditCreatorWallet`, `debitCreatorWallet`, `recordPlatformFee`,
     etc.: `if (process.env.V1_WALLET_FREEZE === 'true') return;`
3. Redeploy. New Stripe events now ONLY write to v2.
