...
- **Deferred — BUG-002 (HIGH, money display):** day P&L on positions opened *today* anchors to previous close, so a position only ever up can show a red "today" loss. Needs the money-path treatment (server `getPortfolio` + snapshot `dailyPL` + tests). Designed in BUGS.md, NOT yet done.
- Smaller open items (BUG-006 duplicated marquee chips, 007 stale empty-state copy, 008 doomed/401 calls for logged-out visitors) + observations (indices bar shows SPY/QQQ/DIA ETF prices labeled as index levels) in BUGS.md.
